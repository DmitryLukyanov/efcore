...
namespace Microsoft.EntityFrameworkCore.Mongo.Update.Internal;
...
public class DocumentSource
{
    ...
    private readonly MongoDatabaseWrapper _database;
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public DocumentSource(IEntityType entityType, MongoDatabaseWrapper database)
    {
        _collectionName = entityType.GetCollectionName()!;
        _database = database;
        _entityType = entityType;
        _idProperty = entityType.GetProperties().FirstOrDefault(p => p.GetJsonPropertyName() == "_id");
        _jObjectProperty = entityType.FindProperty("_t");//StoreKeyConvention.JObjectPropertyName);
    }
...
#pragma warning restore EF1001 // Internal EF Core API usage.
    ...
    private static BsonValue? ConvertPropertyValue(IProperty property, IUpdateEntry entry)
    {
        ...
        return value == null
            ? null
            : (value as BsonValue) ??
            //JToken.FromObject(value, MongoClientWrapper.Serializer);
            BsonValue.Create(value); // BsonValueSerializer?
    }
}

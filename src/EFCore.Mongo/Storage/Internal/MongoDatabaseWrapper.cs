...
using Microsoft.EntityFrameworkCore.Mongo.Update.Internal;
//using Microsoft.EntityFrameworkCore.Mongo.Update.Internal;
...
#pragma warning restore EF1001 // Internal EF Core API usage.
    ...
    private bool Save(IUpdateEntry entry)
    {
        ...
        var collectionName = documentSource.GetCollectionName();
        ...
        switch (state)
        {
            case EntityState.Added:
                ...
                return _mongoClient.CreateItem(collectionName, newDocument, entry);
                ...
                else
                {
                    ...
                    if (propertyName != null)
                    {
                        document[propertyName] =
                            //JToken.FromObject(entityType.GetDiscriminatorValue(), MongoClientWrapper.Serializer);
                            BsonValue.Create(entityType.GetDiscriminatorValue());
                    }
                }

                return _mongoClient.ReplaceItem(
                    collectionName, documentSource.GetId(entry.SharedIdentityEntry ?? entry), document, entry);

            case EntityState.Deleted:
                return _mongoClient.DeleteItem(collectionName, documentSource.GetId(entry), entry);
    ...
    private Task<bool> SaveAsync(IUpdateEntry entry, CancellationToken cancellationToken)
    {
        ...
        switch (state)
        {
            case EntityState.Added:
                ...
                return _mongoClient.CreateItemAsync(
                    collectionName, newDocument, entry, cancellationToken);
            ...
            case EntityState.Modified:
                ...
                else
                {
                    ...
                    if (propertyName != null)
                    {
                        document[propertyName] = BsonValue.Create(entityType.GetDiscriminatorValue());
                            //JToken.FromObject(entityType.GetDiscriminatorValue(), MongoClientWrapper.Serializer);
                    }
                }

                return _mongoClient.ReplaceItemAsync(
                    collectionName,
            ...
            case EntityState.Deleted:
                return _mongoClient.DeleteItemAsync(
                    collectionName, documentSource.GetId(entry), entry, cancellationToken);
        ...
        if (principal == null)
        {
            if (_sensitiveLoggingEnabled)
            {
                throw new InvalidOperationException(
                    "MongoStrings.OrphanedNestedDocumentSensitive"
...
#pragma warning restore EF1001 // Internal EF Core API usage.
    ...
    private Exception WrapUpdateException(Exception exception, IReadOnlyList<IUpdateEntry> entries)
    {
        ...
        //{
        //    MongoException { StatusCode: HttpStatusCode.PreconditionFailed }
        //        => new DbUpdateConcurrencyException("CosmosStrings.UpdateConflict(id), exception, entries"),
        //    MongoException { StatusCode: HttpStatusCode.Conflict }

...
//using EFCore.Mongo.Exttensions.Internal;
...
public class MongoDatabaseCreator : IDatabaseCreator
{
    ...
    public virtual bool EnsureCreated()
    {
        ...
        var created = _mongoClient.CreateDatabaseIfNotExists(/*model.GetThroughput()*/);

        foreach (var collection in GetCollectionsToCreate(model))
        {
            created |= _mongoClient.CreateCollectionIfNotExists(collection);
        }
    ...
    {
        var model = _designTimeModel.Model;
        var created = await _mongoClient.CreateDatabaseIfNotExistsAsync(/*model.GetThroughput(), */cancellationToken)
            .ConfigureAwait(false);

        foreach (var collection in GetCollectionsToCreate(model))
        {
            created |= await _mongoClient.CreateCollectionIfNotExistsAsync(collection, cancellationToken)
                .ConfigureAwait(false);
        }
    ...
    private static IEnumerable<CollectionProperties> GetCollectionsToCreate(IModel model)
    {
        var containers = new Dictionary<string, List<IEntityType>>();
        foreach (var entityType in model.GetEntityTypes().Where(et => et.FindPrimaryKey() != null))
        {
            var container = entityType.GetCollectionName();
            if (container == null)
            {
                continue;
            }
        ...
        foreach (var (collectionName, mappedTypes) in containers)
        {
            ...
            yield return new CollectionProperties(
                collectionName//,
                //partitionKey!,
                //analyticalTtl,
                //defaultTtl/*,
    ...
    public virtual Task<bool> EnsureDeletedAsync(CancellationToken cancellationToken = default)
        => _mongoClient.DeleteDatabaseAsync(cancellationToken);
    ...
    //        : MongoClientWrapper.DefaultPartitionKey;
    //}
}

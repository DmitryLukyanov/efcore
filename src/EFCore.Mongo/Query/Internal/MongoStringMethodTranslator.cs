...
/// </summary>
public class MongoStringMethodTranslator : IMethodCallTranslator
{
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public MongoStringMethodTranslator(IMqlExpressionFactory mqlExpressionFactory)
    {
        ...
        IDiagnosticsLogger<DbLoggerCategory.Query> logger)
    {
        if (instance != null)
        {
            ...
            // Cosmos DB LTRIM does not take arguments
                    && ((arguments[0] as MqlConstantExpression)?.Value as Array)?.Length == 0))
            {
            ...
            {
                return arguments[0] is MqlConstantExpression constant
        ...
        {
            return TranslateSystemFunction("LEFT", typeof(char), arguments[0], _mqlExpressionFactory.Constant(1));
        }

        if (LastOrDefaultMethodInfoWithoutArgs.Equals(method))
        {
            return TranslateSystemFunction("RIGHT", typeof(char), arguments[0], _mqlExpressionFactory.Constant(1));
        }

        if (StringConcatWithTwoArguments.Equals(method))
        {
            return _mqlExpressionFactory.Add(
                arguments[0],
                arguments[1]);
        }
            ...
            || StringComparisonWithComparisonTypeArgumentStatic.Equals(method))
        {
            ...
            if (comparisonTypeArgument is MqlConstantExpression constantComparisonTypeArgument
                ...
                return StringComparisonWithComparisonTypeArgumentInstance.Equals(method)
                    ? comparisonTypeArgumentValue == StringComparison.OrdinalIgnoreCase
                        ? TranslateSystemFunction(
                            "STRINGEQUALS", typeof(bool), instance!, arguments[0], _mqlExpressionFactory.Constant(true))
                        : TranslateSystemFunction("STRINGEQUALS", typeof(bool), instance!, arguments[0])
                    : comparisonTypeArgumentValue == StringComparison.OrdinalIgnoreCase
                        ? TranslateSystemFunction(
                            "STRINGEQUALS", typeof(bool), arguments[0], arguments[1], _mqlExpressionFactory.Constant(true))
    ...
    private MqlExpression TranslateSystemFunction(string function, Type returnType, params MqlExpression[] arguments)
        => _mqlExpressionFactory.Function(function, arguments, returnType);
}

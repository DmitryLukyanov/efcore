...
public class SelectExpression : Expression
{
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public SelectExpression(IEntityType entityType, string mql, Expression argument)
    {
        CollectionName = entityType.GetCollectionName();
        FromExpression = new FromMqlExpression(entityType, RootAlias, mql, argument);
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public virtual MqlExpression Predicate { get; private set; }
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public virtual MqlExpression Limit { get; private set; }
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public virtual MqlExpression Offset { get; private set; }
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public virtual int AddToProjection(MqlExpression sqlExpression)
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public virtual void ApplyPredicate(MqlExpression expression)
    {
        if (expression is MqlConstantExpression sqlConstant
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public virtual void ApplyLimit(MqlExpression sqlExpression)
    {
        if (Limit != null)
        {
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public virtual void ApplyOffset(MqlExpression mqlExpression)
    {
        if (Limit != null
            || Offset != null)
        {
    ...
    protected override Expression VisitChildren(ExpressionVisitor visitor)
    {
        ...
        var predicate = (MqlExpression)visitor.Visit(Predicate);
        changed |= predicate != Predicate;

        var orderings = new List<OrderingExpression>();
        foreach (var ordering in _orderings)
        {
            var orderingExpression = (MqlExpression)visitor.Visit(ordering.Expression);
            changed |= orderingExpression != ordering.Expression;
            orderings.Add(ordering.Update(orderingExpression));
        }

        var offset = (MqlExpression)visitor.Visit(Offset);
        changed |= offset != Offset;

        var limit = (MqlExpression)visitor.Visit(Limit);
        changed |= limit != Limit;
    ...
    public virtual SelectExpression Update(
        List<ProjectionExpression> projections,
        RootReferenceExpression fromExpression,
        MqlExpression? predicate,
        List<OrderingExpression>? orderings,
        MqlExpression? limit,
        MqlExpression? offset)
    {
        ...
        return new SelectExpression(projections, fromExpression, orderings, CollectionName)
        {
            _projectionMapping = projectionMapping,
            Predicate = predicate,
            Offset = offset,

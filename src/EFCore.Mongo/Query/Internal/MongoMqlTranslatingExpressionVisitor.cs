...
/// </summary>
public class MongoMqlTranslatingExpressionVisitor : ExpressionVisitor
{
    ...
    private static readonly MethodInfo ParameterValueExtractorMethod =
        typeof(MongoMqlTranslatingExpressionVisitor).GetTypeInfo().GetDeclaredMethod(nameof(ParameterValueExtractor));

    private static readonly MethodInfo ParameterListValueExtractorMethod =
        typeof(MongoMqlTranslatingExpressionVisitor).GetTypeInfo().GetDeclaredMethod(nameof(ParameterListValueExtractor));
    ...
    private readonly MqlTypeMappingVerifyingExpressionVisitor _mqlVerifyingExpressionVisitor;
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public MongoMqlTranslatingExpressionVisitor(
        ...
        IMethodCallTranslatorProvider methodCallTranslatorProvider)
    {
        ...
        _mqlVerifyingExpressionVisitor = new MqlTypeMappingVerifyingExpressionVisitor();
    }
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public virtual MqlExpression Translate(Expression expression)
    {
    ...
    private MqlExpression TranslateInternal(Expression expression)
    {
        var result = Visit(expression);

        if (result is MqlExpression translation)
        {
            translation = _mqlExpressionFactory.ApplyDefaultTypeMapping(translation);
            ...
            _mqlVerifyingExpressionVisitor.Visit(translation);
    ...
    protected override Expression VisitBinary(BinaryExpression binaryExpression)
    {
        ...
        return TranslationFailed(binaryExpression.Left, visitedLeft, out var sqlLeft)
            || TranslationFailed(binaryExpression.Right, visitedRight, out var sqlRight)
                ? null
                : _mqlExpressionFactory.MakeBinary(
        ...
        Expression ProcessGetType(EntityReferenceExpression entityReferenceExpression, Type comparisonType, bool match)
        {
            ...
            {
                // No hierarchy
                return _mqlExpressionFactory.Constant((entityType.ClrType == comparisonType) == match);
            }

            if (entityType.GetAllBaseTypes().Any(e => e.ClrType == comparisonType))
            {
                // EntitySet will never contain a type of base type
                return _mqlExpressionFactory.Constant(!match);
            }
            ...
            if (derivedType != null)
            {
                ...
                if (TryBindMember(entityReferenceExpression, MemberIdentity.Create(entityType.GetDiscriminatorPropertyName()))
                    is MqlExpression discriminatorColumn)
                {
                    return match
                        ? _mqlExpressionFactory.Equal(
                            discriminatorColumn,
                            _mqlExpressionFactory.Constant(derivedType.GetDiscriminatorValue()))
                        : _mqlExpressionFactory.NotEqual(
                            discriminatorColumn,
                            _mqlExpressionFactory.Constant(derivedType.GetDiscriminatorValue()));
                }
            }
    ...
    protected override Expression VisitConditional(ConditionalExpression conditionalExpression)
    {
        ...
        return TranslationFailed(conditionalExpression.Test, test, out var sqlTest)
            ...
            || TranslationFailed(conditionalExpression.IfFalse, ifFalse, out var sqlIfFalse)
                ? null
                : _mqlExpressionFactory.Condition(sqlTest, sqlIfTrue, sqlIfFalse);
    }
    ...
    protected override Expression VisitConstant(ConstantExpression constantExpression)
        => new MqlConstantExpression(constantExpression, null);
    ...
    protected override Expression VisitMethodCall(MethodCallExpression methodCallExpression)
    {
        ...
        MqlExpression sqlObject = null;
        MqlExpression[] arguments;
            ...
            && methodCallExpression.Arguments.Count == 1)
        {
            ...
            if (left is MqlExpression leftSql
                && right is MqlExpression rightSql)
            {
                sqlObject = leftSql;
                 ...
                 && method.GetGenericMethodDefinition().Equals(EnumerableMethods.Contains))
        {
            ...
            if (enumerable is MqlExpression sqlEnumerable
                && item is MqlExpression sqlItem)
            {
        ...
        else
        {
            ...
            arguments = new MqlExpression[methodCallExpression.Arguments.Count];
        ...
        if (translation == null)
        {
            ...
            else
            {
                AddTranslationErrorDetails(
                    "CoreStrings.QueryUnableToTranslateMethod(methodCallExpression.Method.DeclaringType?.DisplayName(),methodCallExpression.Method.Name)");
            }
        }
    ...
    protected override Expression VisitParameter(ParameterExpression parameterExpression)
        => parameterExpression.Name?.StartsWith(QueryCompilationContext.QueryParameterPrefix, StringComparison.Ordinal) == true
            ? new MqlParameterExpression(parameterExpression, null)
            : null;
    ...
    protected override Expression VisitUnary(UnaryExpression unaryExpression)
    {
        ...
        switch (unaryExpression.NodeType)
        {
            case ExpressionType.Not:
                return _mqlExpressionFactory.Not(sqlOperand);

            case ExpressionType.Negate:
            case ExpressionType.NegateChecked:
                return _mqlExpressionFactory.Negate(sqlOperand);
    ...
    protected override Expression VisitTypeBinary(TypeBinaryExpression typeBinaryExpression)
    {
            ...
            && innerExpression is EntityReferenceExpression entityReferenceExpression)
        {
            ...
            {
                return _mqlExpressionFactory.Constant(true);
            }
            ...
            if (derivedType != null
                && TryBindMember(
                    ...
                    MemberIdentity.Create(entityType.GetDiscriminatorPropertyName())) is MqlExpression discriminatorColumn)
            {
                ...
                return concreteEntityTypes.Count == 1
                    ? _mqlExpressionFactory.Equal(
                        discriminatorColumn,
                        _mqlExpressionFactory.Constant(concreteEntityTypes[0].GetDiscriminatorValue()))
                    : _mqlExpressionFactory.In(
                        discriminatorColumn,
                        _mqlExpressionFactory.Constant(concreteEntityTypes.Select(et => et.GetDiscriminatorValue()).ToList()),
                        negated: false);
            }
        }
    ...
    private Expression TryBindMember(Expression source, MemberIdentity member)
    {
        ...
        {
            AddTranslationErrorDetails(
                "CoreStrings.QueryUnableToTranslateMember(member.Name,entityReferenceExpression.EntityType.DisplayName())");
        }
    ...
    private bool TryRewriteContainsEntity(Expression source, Expression item, out Expression result)
    {
        ...
        if (primaryKeyProperties == null)
        {
            throw new InvalidOperationException(
                "CoreStrings.EntityEqualityOnKeylessEntityNotSupported(nameof(Queryable.Contains), entityType.DisplayName())");
        }
        ...
        switch (source)
        {
            case MqlConstantExpression mqlConstantExpression:
                var values = (IEnumerable)mqlConstantExpression.Value;
            ...
            case MqlParameterExpression mqlParameterExpression
                when mqlParameterExpression.Name.StartsWith(QueryCompilationContext.QueryParameterPrefix, StringComparison.Ordinal):
                var lambda = Expression.Lambda(
                    Expression.Call(
                        ParameterListValueExtractorMethod.MakeGenericMethod(entityType.ClrType, property.ClrType.MakeNullable()),
                        QueryCompilationContext.QueryContextParameter,
                        Expression.Constant(mqlParameterExpression.Name, typeof(string)),
                ...
                var newParameterName =
                    ...
                    + $"{mqlParameterExpression.Name[QueryCompilationContext.QueryParameterPrefix.Length..]}_{property.Name}";
        ...
        out Expression result)
    {
        ...
        {
            result = _mqlExpressionFactory.Constant(false);
            return true;
        }
        ...
        switch (target)
        {
            case MqlConstantExpression mqlConstantExpression:
                return Expression.Constant(
                    property.GetGetter().GetClrValue(mqlConstantExpression.Value), property.ClrType.MakeNullable());
    ...
    private static bool IsNullSqlConstantExpression(Expression expression)
        => expression is MqlConstantExpression mqlConstant && mqlConstant.Value == null;

    private static MqlConstantExpression GetConstantOrNull(Expression expression)
        => CanEvaluate(expression)
            ? new MqlConstantExpression(
                Expression.Constant(
...
#pragma warning restore IDE0066 // Convert switch statement to expression
    ...
    [DebuggerStepThrough]
    private static bool TranslationFailed(Expression original, Expression translation, out MqlExpression castTranslation)
    {
        if (original != null
            && !(translation is MqlExpression))
        {
            castTranslation = null;
            return true;
        }

        castTranslation = translation as MqlExpression;
        return false;
    }
    ...
    private sealed class MqlTypeMappingVerifyingExpressionVisitor : ExpressionVisitor
    {
        protected override Expression VisitExtension(Expression extensionExpression)
        {
            if (extensionExpression is MqlExpression sqlExpression

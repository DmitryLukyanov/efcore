...
using Microsoft.EntityFrameworkCore.Mongo.Metadata.Internal;
...
{
    private abstract class MongoProjectionBindingRemovingExpressionVisitorBase : System.Linq.Expressions.ExpressionVisitor
    {
        private static readonly MethodInfo GetItemMethodInfo
            = typeof(BsonDocument).GetRuntimeProperties()
                ...
                .Single(pi => pi.Name == "Item" && pi.GetIndexParameters().Length == 1 && pi.GetIndexParameters()[0].ParameterType == typeof(string))
                .GetMethod;

        private static readonly PropertyInfo JTokenTypePropertyInfo
            = typeof(BsonValue).GetRuntimeProperties()
                .FirstOrDefault(mi => mi.Name == "nameof(BsonValue.Type)"); // TODO

        private static readonly MethodInfo JTokenToObjectWithSerializerMethodInfo
            = typeof(BsonValue).GetRuntimeMethods()
                .FirstOrDefault(mi => mi.Name == "nameof(BsonDocument.ToObject) && mi.GetParameters().Length == 1 && mi.IsGenericMethodDefinition");
        ...
        private static readonly MethodInfo ToObjectWithSerializerMethodInfo
            = typeof(MongoProjectionBindingRemovingExpressionVisitorBase)
                .GetRuntimeMethods().Single(mi => mi.Name == nameof(SafeToObjectWithSerializer));

        protected MongoProjectionBindingRemovingExpressionVisitorBase(
            ...
            switch (extensionExpression)
            {
                case ProjectionBindingExpression projectionBindingExpression:
                {
                    ...
                    return CreateGetValueExpression(
                        _jObjectParameter,
                        projection.Alias,
                        projectionBindingExpression.Type, (projection.Expression as MqlExpression)?.TypeMapping);
                }
...
#pragma warning restore EF1001 // Internal EF Core API usage.
        ...
        private static readonly MethodInfo IncludeReferenceMethodInfo
            = typeof(MongoProjectionBindingRemovingExpressionVisitorBase).GetTypeInfo()
...
#pragma warning restore EF1001 // Internal EF Core API usage.
        ...
        {
            var properties = typeof(BsonDocument).GetRuntimeProperties().ToList();
            var res = properties.Select(c=> new { n = c.Name, i = c.GetIndexParameters().ToList()});
            return Expression.Call(jObjectExpression, GetItemMethodInfo, Expression.Constant(propertyName));
        }
        ...
        private static T SafeToObject<T>(BsonDocument token)
            => token == null || token.IsBsonNull ? default : BsonSerializer.Deserialize<T>(token /*, MongoClientWrapper.Serializer*/);

        private static T SafeToObjectWithSerializer<T>(BsonValue token)
        {
            if (token == null || token.IsBsonNull)
            {
                return default;
            }
            else
            {
                using (var memory = new MemoryStream(token.ToBson()))
                using (var jsonReader = new BsonBinaryReader(memory, new BsonBinaryReaderSettings()))
                {
                    var context = BsonDeserializationContext.CreateRoot(jsonReader);
                    return IBsonSerializerExtensions.Deserialize<T>(BsonSerializer.LookupSerializer<T>(), context);
                }
            }
        }
    }
